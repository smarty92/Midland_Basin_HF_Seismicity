...
class PoroElasModel_HF:
    ...
    def select_eq_datafame(self,df_eq_select,df_ope,eq_time_years,dist_thresh,end_production=None):
        ...
        else: end_inj = self.datetime_to_years(pd.to_datetime(dataframe_ope['JobEndDate']))
   ...
   ###############
    ...
    ## main function to calculate poroelastic stresses due to a displacement discontinuity - equation E.144 page 836 (Cheng)
    ## note I did not include the heaviside function since I set thau = 0 
    def poroelastic_stresses_drained(self,mu,poi_u,S,etha,diffu,x,y,z,k,l,t,thau):
    ...
    def poroelastic_stresses_drained_mat(self,mu,poi_u,S,etha,diffu,x,y,z,k,l,t,thau):
        ...
        eps = np.outer(r, 1 / np.sqrt(4*diffu*(t - thau)))
        ...
        stress_mat = np.zeros([len(r),6,len(t)])
    ...
    def pore_pressure_drained_DC(self,S,etha,diffu,x,y,z,k,l,t,thau):
    ...
    def pore_pressure_drained_DC_mat(self,S,etha,diffu,x,y,z,k,l,t,thau):
    ...
    def interp_prod(self,vectime,vecrate):
        ...
        newvecx= np.arange(d_.min(),d_.max(),7/365.25)
    ...
    def clean_catalog(self,dataframe_):
        ...
        for i in range(len(df_hf)):
            ...
            if comple_year[i] == end_inj_year[i] : 
                df_hf.loc[i,'completion_date'] = df_hf.loc[i,'completion_date']
            else:
                toto = 0
        return df_hf 
    ...
    def compute_CS_cheng_drained(self,dataframe_injection,grid,mecha_param,number_fracture,width,height,time_vector,depth,prod_inj,
        ...
        time_fractures = np.linspace(start_inj,end_inj,number_fracture)
        vol_per_frac = tot_inj_vol / number_fracture
        ...
        try:
            vol_produced_per_day = np.gradient(prod_inj[1])
        except:
            toto = 0
    ...
    #### define the vector that contain time and injected volume for flowback and produciton

            time_vector_flowback   = np.arange(end_inj,completion_end,7/365.25)
            tot_vol_flow_back = tot_inj_vol * ratio_flowback
            fb_vol_per_stage =  tot_vol_flow_back / len(time_vector_flowback)
            fb_vol_per_stage_per_frac =  fb_vol_per_stage / number_fracture
            
                
            ##### for time between start_inj and end_inj

            for i in range(len(time_fractures)):
                
                t_tmp = time_fractures[i]
                ind_tmp = np.where(time_vector_years >= t_tmp)[0]              
                t = 1 + (time_vector_years[ind_tmp] - t_tmp)*year_to_sec
                thau = 0
                center_ = np.array([x_loc_fracture[i],0,well_depth])
                x = cart_grid_rotate[:,0] - center_[0]
                y = cart_grid_rotate[:,1] - center_[1]
                z = cart_grid_rotate[:,2] - center_[2]
                sol_ = vol_per_frac * self.poroelastic_stresses_drained_mat(mu,poi_u,S,etha,diffu,x,y,z,k,l,t,thau)
                output_[:,:,ind_tmp] += sol_
            
     ########## flowbackkkkk
            
            for i in range(len(time_vector_flowback)):
                
                t_tmp = time_vector_flowback[i]
                ind_tmp = np.where(time_vector_years >= t_tmp)[0]
                t = 1 + (time_vector_years[ind_tmp] - t_tmp)*year_to_sec
                thau = 0
                for j in range(number_fracture):
                    center_ = np.array([x_loc_fracture[j],0,well_depth])
                    x = cart_grid_rotate[:,0] - center_[0]
                    y = cart_grid_rotate[:,1] - center_[1]
                    z = cart_grid_rotate[:,2] - center_[2]
                    sol_ = fb_vol_per_stage_per_frac * self.poroelastic_stresses_drained_mat(mu,poi_u,S,etha,diffu,x,y,z,k,l,t,thau)
                    output_[:,:,ind_tmp] += sol_
     ...
     ########### production
            ...
            if prod_inj[1].sum() > 0:
                for i in range(len(prod_inj[0])):
                    t_tmp = prod_inj[0][i]
                    ind_tmp = np.where(time_vector_years >= t_tmp)[0]
                    t = 1 + (time_vector_years[ind_tmp] - t_tmp)*year_to_sec
                    thau = 0
                    prod_vol_per_frac = vol_produced_per_day[i]/number_fracture
                    prod_vol_per_frac = prod_vol_per_frac/1e3
                    prodwater = prod_inj[1][i]/1e3
                    
                    if (prodwater <= (1-ratio_proppant) * (1-ratio_flowback) * tot_inj_vol) :
                        for j in range(number_fracture):
                            center_ = np.array([x_loc_fracture[j],0,well_depth])
                            x = cart_grid_rotate[:,0] - center_[0]
                            y = cart_grid_rotate[:,1] - center_[1]
                            z = cart_grid_rotate[:,2] - center_[2]
                            sol_ = prod_vol_per_frac * self.poroelastic_stresses_drained_mat(mu,poi_u,S,etha,diffu,x,y,z,k,l,t,thau)
                            output_[:,:,ind_tmp] += sol_
    ...
    #### define the vector that contain time and injected volume for flowback and produciton
            ...
            time_vector_flowback   = np.arange(end_inj,start_prod,7/365.25)
            tot_vol_flow_back = tot_inj_vol * ratio_flowback
            fb_vol_per_stage =  tot_vol_flow_back / len(time_vector_flowback)
            fb_vol_per_stage_per_frac =  fb_vol_per_stage / number_fracture
                ##### for time between start_inj and end_inj
     ...
     ########### production
        ...
        final_output_ = np.zeros(output_.shape)
        for i in range(output_.shape[2]):
            final_output_[:,:,i] = self.stress_rot(output_[:,:,i],-rotation_angle)
        return final_output_


    def compute_pore_pressure_drained(self,dataframe_injection,grid,mecha_param,number_fracture,width,height,time_vector,depth,prod_inj,
                                   ratio_flowback,ratio_proppant,flag):
    ...
    #### define the vector that contain time and injected volume for flowback and produciton
            ...
            for i in range(len(time_fractures)):
                ...
                y = cart_grid_rotate[:,1] - center_[1]
                z = cart_grid_rotate[:,2] - center_[2]
                sol_ = vol_per_frac * self.pore_pressure_drained_DC_mat(S,etha,diffu,x,y,z,k,l,t,thau) 
                output_[:,ind_tmp] += sol_
     ...
     ########## flowbackkkkk
            ...
            for i in range(len(time_vector_flowback)):
                ...
                for j in range(number_fracture):
                    ...
                    y = cart_grid_rotate[:,1] - center_[1]
                    z = cart_grid_rotate[:,2] - center_[2]
                    sol_ = fb_vol_per_stage_per_frac * self.pore_pressure_drained_DC_mat(S,etha,diffu,x,y,z,k,l,t,thau) 
                    output_[:,ind_tmp] -= sol_
                ...
                for i in range(len(prod_inj[0])):
                    ...
                    if (prodwater <= (1-ratio_proppant) * (1-ratio_flowback) * tot_inj_vol) :
                        for j in range(number_fracture):
                            ...
                            y = cart_grid_rotate[:,1] - center_[1]
                            z = cart_grid_rotate[:,2] - center_[2]
                            sol_ = prod_vol_per_frac * self.pore_pressure_drained_DC_mat(S,etha,diffu,x,y,z,k,l,t,thau) 
                            output_[:,ind_tmp] -= sol_
